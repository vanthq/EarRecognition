...
best_acc1 = 0
rizese = [84, 72, 60, 48, 36, 24, 12]


# rizese =[32,30,28,26,24,22,20,18,16,14,12,10]
...
def main(se):
    ...
    if args.multiprocessing_distributed:
        ...
        main_worker(args.gpu, ngpus_per_node, args, se)


def main_worker(gpu, ngpus_per_node, args, se):
    global best_acc1
    args.gpu = gpu

    if not os.path.exists(args.result_path + str(se)):
        os.makedirs(args.result_path + str(se))

    if not args.evaluate:
        if args.resume:
            with open(os.path.join(args.result_path + str(se), 'resume_args.json'), 'a') as f:
                json.dump(vars(args), f)
        else:
            with open(os.path.join(args.result_path + str(se), 'args.json'), 'w') as f:
                json.dump(vars(args), f)
    ...
    if args.resume:
        if os.path.isfile('C:\\Users\\86152\\Desktop\\results\\model_best.pth.tar'):
            print("=> loading checkpoint '{}'".format('C:\\Users\\86152\\Desktop\\results\\model_best.pth.tar'))
            checkpoint = torch.load('C:\\Users\\86152\\Desktop\\results\\model_best.pth.tar')
            ...
            print("=> loaded checkpoint '{}' (epoch {})"
                  .format('C:\\Users\\86152\\Desktop\\results\\model_best.pth.tar', checkpoint['epoch']))
            if args.lr_scheduler == 'MultiStepLR':
    ...
    #     train_sampler = None
    #
    # train_loader, val_loader = ImageProduct.split_Train_Val_Data(data_dir, [0.8, 0.2])
    ...
    image_path = os.path.join(data_root, "data_set", "flower_data")  # flower_data5：5    images2：8 flower_data_raw为原始图片
    assert os.path.exists(image_path), "{} path does not exist.".format(image_path)

    # ImageFolder自动会把单通道图片转为3通道
    ...
    train_logger = Logger(os.path.join(args.result_path + str(se), 'train.log'),
                          ['epoch', 'loss', 'acc1', 'acc5', 'lr'], mode=mode)

    val_logger = Logger(os.path.join(args.result_path + str(se), 'val.log'), ['epoch', 'loss', 'acc1', 'acc5'],
                        mode=mode)
